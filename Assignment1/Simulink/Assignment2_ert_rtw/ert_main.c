...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Assignment2_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Assignment2_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Assignment2_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Assignment2_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Assignment2_M, 0);

  /* Initialize model */
  Assignment2_initialize();
