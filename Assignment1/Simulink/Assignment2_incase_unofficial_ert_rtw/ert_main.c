...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Assignment2_incase_unofficia_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Assignment2_incase_unofficial_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Assignment2_incase_unofficia_M) ==
                       (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Assignment2_incase_unofficia_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Assignment2_incase_unofficial_terminate();
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Assignment2_incase_unofficia_M, 0);

  /* Initialize model */
  Assignment2_incase_unofficial_initialize();
