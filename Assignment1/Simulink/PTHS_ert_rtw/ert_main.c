...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(PTHS_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    PTHS_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(PTHS_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(PTHS_M, "stopping the model");
  runModel = 0;
}
