...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(AdaptivePacing_2_M) == (NULL)) &&
    !rtmGetStopRequested(AdaptivePacing_2_M);
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(AdaptivePacing_2_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(AdaptivePacing_2_M, true);
      }

      if (rtmGetStopRequested(AdaptivePacing_2_M) == true) {
        rtmSetErrorStatus(AdaptivePacing_2_M, "Simulation finished");
        break;
      }
    }

    AdaptivePacing_2_step();

    /* Get model outputs here */
    rtExtModeCheckEndTrigger();
    stopRequested = !((rtmGetErrorStatus(AdaptivePacing_2_M) == (NULL)) &&
                      !rtmGetStopRequested(AdaptivePacing_2_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(AdaptivePacing_2_M, "stopping the model");
  runModel = 0;
}
...
void *backgroundTask(void *arg)
{
  while (runModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(AdaptivePacing_2_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(AdaptivePacing_2_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(AdaptivePacing_2_M, 0);
  ...
  /* Initialize model */
  AdaptivePacing_2_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(AdaptivePacing_2_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(AdaptivePacing_2_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(AdaptivePacing_2_M, true);
    }
  }
