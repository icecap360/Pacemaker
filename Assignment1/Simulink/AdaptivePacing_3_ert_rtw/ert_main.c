...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(AdaptivePacing_3_M) == (NULL)) &&
    !rtmGetStopRequested(AdaptivePacing_3_M);
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(AdaptivePacing_3_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(AdaptivePacing_3_M, true);
      }

      if (rtmGetStopRequested(AdaptivePacing_3_M) == true) {
        rtmSetErrorStatus(AdaptivePacing_3_M, "Simulation finished");
        break;
      }
    }

    AdaptivePacing_3_step();

    /* Get model outputs here */
    rtExtModeCheckEndTrigger();
    stopRequested = !((rtmGetErrorStatus(AdaptivePacing_3_M) == (NULL)) &&
                      !rtmGetStopRequested(AdaptivePacing_3_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(AdaptivePacing_3_M, "stopping the model");
  runModel = 0;
}
...
void *backgroundTask(void *arg)
{
  while (runModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(AdaptivePacing_3_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(AdaptivePacing_3_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(AdaptivePacing_3_M, 0);
  ...
  /* Initialize model */
  AdaptivePacing_3_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(AdaptivePacing_3_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(AdaptivePacing_3_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(AdaptivePacing_3_M, true);
    }
  }
